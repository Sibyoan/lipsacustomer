...
const priceRanges = [
  { price: '349', label: 'Under', color1: '#0b4a6f', color2: '#1e5a7a', products: ['🛋️','🪑','🏺'] },
  { price: '449', label: 'Under', color1: '#0b4a6f', color2: '#1e5a7a', products: ['🍳','🥄','🫙'] },
  { price: '499', label: 'Under', color1: '#0b4a6f', color2: '#1e5a7a', products: ['📱','🔌','🎧'] },
  { price: '999', label: 'Under', color1: '#0b4a6f', color2: '#1e5a7a', products: ['🎮','📷','⌚'] },
  { price: '1000', label: 'Above', color1: '#0b4a6f', color2: '#1e5a7a', products: ['💻','📺','🔭'] },
];
...
export default function PriceRangeBanner() {
  return (
      ...
      <div className="max-w-[1200px] mx-auto px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {priceRanges.map((item, i) => (
            <PriceCard key={i} price={item.price} label={item.label} color1={item.color1} color2={item.color2} products={item.products} />
          ))}
        </div>
      </div>
    </section>
